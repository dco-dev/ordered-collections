(ns com.dean.ordered-collections.memory-test
  "Memory overhead analysis for ordered-collections.
   ...
   Run with: lein test :only com.dean.ordered-collections.memory-test"
...
(deftest node-structure-analysis
  (testing "Individual node memory breakdown"
    (let [;; Create minimal collections to measure node overhead
          ...
          set-root (.getRoot ^com.dean.ordered_collections.tree.root.INodeCollection small-set)
          map-root (.getRoot ^com.dean.ordered_collections.tree.root.INodeCollection small-map)]
